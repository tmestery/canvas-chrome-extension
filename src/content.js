(() => {
  ...
  function getGrades() {
    ...
    document.querySelectorAll(".ic-DashboardCard").forEach(card => {
      ...
      if (title && grade) results.push({ title, grade });
    });
    return results;
  }
  ...
  function getUpcoming() {
    ...
    document.querySelectorAll(".PlannerItem").forEach(item => {
      ...
      if (title && date) results.push({ title, date });
    });
    return results;
  }
  ...
  function createCanvasPanel() {
    ...
    style.textContent = `
      ...
      #canvas-helper header {
        ...
        font-size: 14px;
        font-weight: 600;
        background: linear-gradient(135deg, var(--canvas-accent), var(--canvas-accent-dark));
        display: flex;
      ...
      #canvas-helper-toggle:hover { opacity: 1; }
      #canvas-helper section {
        padding: 14px 16px;
        border-bottom: 1px solid var(--canvas-border);
      }
      #canvas-helper section:last-child { border-bottom: none; }
      #canvas-helper h3 {
        margin: 0 0 10px;
        font-size: 11px;
        letter-spacing: .12em;
      ...
      .canvas-item { display: flex; flex-direction: column; gap: 2px; padding: 8px 0; }
      .canvas-item span { font-size: 13px; line-height: 1.35; color: var(--canvas-text); }
      .canvas-item strong { font-size: 12px; font-weight: 500; color: var(--canvas-accent-soft); }
      .canvas-muted { font-size: 11px; color: var(--canvas-muted); }
      #grades-list, #assignments-list { display: flex; flex-direction: column; }
    `;
    ...
    document.getElementById("canvas-helper-toggle").onclick = () => panel.remove();
  ...
  // Inject panel on first load
  createCanvasPanel();

  (() => {
    function handleRouteChange() {
      const panel = document.getElementById("canvas-helper");
      if (panel) panel.remove();
      createCanvasPanel();
    }
    window.addEventListener("popstate", handleRouteChange);
    const pushOrig = history.pushState;
    history.pushState = function (...args) { pushOrig.apply(this, args); handleRouteChange(); };
    const replaceOrig = history.replaceState;
    history.replaceState = function (...args) { replaceOrig.apply(this, args); handleRouteChange(); };
  })();
